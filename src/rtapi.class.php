<?php
/**
 * RTAPI / GAPI - Google Analytics PHP Interface
 * 
 ...
 * 
 * @author BROU Assouman Guy Charles <guy-charles@hotmail.com>
 * @version 1.0
 * 
 */
...
class rtapi {
  ...
  const report_data_url = 'https://www.googleapis.com/analytics/v3/data/realtime';
  ...
  public function requestAccountData($start_index=1, $max_results=1000) {
    ...
    $url = new rtapiRequest(rtapi::account_data_url);
  ...
  public function requestReportData($report_id, $dimensions=null, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=10000) {
    ...
    if (is_array($dimensions)) {
      ...
      foreach ($dimensions as $dimesion) {
        $dimensions_string .= ',rt:' . $dimesion;
      }
      $parameters['dimensions'] = substr($dimensions_string, 1);
    } elseif ($dimensions !== null) {
      $parameters['dimensions'] = 'rt:'.$dimensions;
    }

    if (is_array($metrics)) {
      $metrics_string = '';
      foreach ($metrics as $metric) {
        $metrics_string .= ',rt:' . $metric;
      }
      $parameters['metrics'] = substr($metrics_string, 1);
    } else {
      $parameters['metrics'] = 'rt:'.$metrics;
    }
    ...
    if ($sort_metric==null&&isset($parameters['metrics'])) {
      ...
      foreach ($sort_metric as $sort_metric_value) {
        ...
        if (substr($sort_metric_value, 0, 1) == "-") {
          $sort_metric_string .= ',-rt:' . substr($sort_metric_value, 1); // Descending
        }
        else {
          $sort_metric_string .= ',rt:' . $sort_metric_value; // Ascending
        }
      }
      ...
      if (substr($sort_metric, 0, 1) == "-") {
        $parameters['sort'] = '-rt:' . substr($sort_metric, 1);
      } else {
        $parameters['sort'] = 'rt:' . $sort_metric;
      }
    }
    ...
    $parameters['prettyprint'] = rtapi::dev_mode ? 'true' : 'false';
    
    $url = new rtapiRequest(rtapi::report_data_url);
   ...
   * @return Array of rtapiAccountEntry objects
   */
  protected function accountObjectMapper($json_string) {
    ...
    foreach ($json['items'] as $item) {
      foreach ($item['webProperties'] as $property) {
        ...
        $results[] = new rtapiAccountEntry($property);
      }
    }
   ...
   *
   * @param String $json_string
   * @return Array of rtapiReportEntry objects
   */
  protected function reportObjectMapper($json_string) {
    ...
    foreach($json['totalsForAllResults'] as $index => $metric_value) {
      ...
      if (preg_match('/^(\d+\.\d+)|(\d+E\d+)|(\d+.\d+E\d+)$/', $metric_value)) {
        $report_aggregate_metrics[str_replace('rt:', '', $index)] = floatval($metric_value);
      } else {
        $report_aggregate_metrics[str_replace('rt:', '', $index)] = intval($metric_value);
      }
    }
        ...
        foreach($json['columnHeaders'] as $index => $header) {
          switch($header['columnType']) {
            case 'METRIC':
              ...
              //Check for float, or value with scientific notation
              if(preg_match('/^(\d+\.\d+)|(\d+E\d+)|(\d+.\d+E\d+)$/',$metric_value)) {
                $metrics[str_replace('rt:', '', $header['name'])] = floatval($metric_value);
              } else {
                $metrics[str_replace('rt:', '', $header['name'])] = intval($metric_value);
              }
              break;
            case 'DIMENSION':
  ...
  public function __call($name, $parameters) {
    ...
    $parameter_key = rtapi::ArrayKeyExists($name, $this->report_root_parameters);

    if ($parameter_key) {
      return $this->report_root_parameters[$parameter_key];
    }

    $aggregate_metric_key = rtapi::ArrayKeyExists($name, $this->report_aggregate_metrics);
 ...
 *
 */
class rtapiAccountEntry {
  private $properties = array();

  /**
   * Constructor function for all new rtapiAccountEntry instances
   * 
   * @param Array $properties
   * @return rtapiAccountEntry
   */
  ...
  public function __call($name, $parameters) {
    ...
    $property_key = rtapi::ArrayKeyExists($name, $this->properties);
 ...
 *
 */
class rtapiReportEntry {
  private $metrics = array();
  private $dimensions = array();

  /**
   * Constructor function for all new rtapiReportEntry instances
   * 
   * @param Array $metrics
   * @param Array $dimensions
   * @return rtapiReportEntry
   */
...
class rtapiOAuth2 {
  ...
  public function fetchToken($client_email, $key_file, $delegate_email = null) {
    ...
    $url = new rtapiRequest(self::request_url);
...
class rtapiRequest {
  ...
  const interface_name = rtapi::interface_name;
